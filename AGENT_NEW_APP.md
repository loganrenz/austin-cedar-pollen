...
## 2. Prerequisites in nuxt_template Doppler (config: base)

The setup script copies shared keys from the **nuxt_template** Doppler project. These must be set **before** running `setup:new-app`:

| Secret | Format | Purpose |
|--------|--------|---------|
| `GA_ACCOUNT_ID` | numeric ID | Google Analytics account ID for creating GA4 properties per app. |
| `GSC_SERVICE_ACCOUNT_JSON` | base64 | Google service account JSON for GSC automation (encode with `node scripts/doppler-b64-key.mjs ./key.json`). |
| `GSC_USER_EMAIL` | email | Email to grant GSC ownership to. |
| **`POSTHOG_PUBLIC_KEY`** | `phc_...` | PostHog **project API key**. All apps share one PostHog project; events are partitioned by `APP_NAME` (filter by property `app` in PostHog). **If missing, PostHog tracking is silently disabled.** Find it in PostHog: Settings > Project > Project API Key. |
| `POSTHOG_PERSONAL_API_KEY` | `phx_...` | PostHog personal API key for management APIs (not used by the client SDK). |

## 3. Checkout the template with gh
...
## 5. Run the two finish commands (required)
...
2. **GSC verify** (use the same SITE_URL as in step 4; quote it for the shell):
   ```bash
...
## 6. If the user needs more detail
...
1. Ensure **nuxt_template** Doppler has all required secrets (see step 2), especially `POSTHOG_PUBLIC_KEY`.
2. Get **APP_NAME** and **SITE_URL**.
3. Checkout template: `gh repo clone loganrenz/nuxt-v4-template <APP_NAME>` then `cd <APP_NAME>`.
4. Run: `npm run setup:new-app -- <APP_NAME> <SITE_URL>`.
5. **You must run or hand off:** `doppler run -- npm run deploy`, then (after production is live) `SITE_URL="<their SITE_URL>" doppler run --project nuxt_template --config base -- npm run setup:gsc:verify`. If GSC verify fails with "verification token could not be found", wait 1–2 min or redeploy, then retry.

No .env is used; all secrets are in Doppler (shared nuxt_template for setup, per-app Doppler for the app's own SITE_URL, GA_MEASUREMENT_ID, and POSTHOG_PUBLIC_KEY).
