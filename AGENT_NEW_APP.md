...
4. **You must run or hand off:** `doppler run -- npm run deploy`, then `SITE_URL="<their SITE_URL>" doppler run --project nuxt_template --config base -- npm run setup:gsc:verify`. Give the user the exact two commands with their real SITE_URL so they can finish.
